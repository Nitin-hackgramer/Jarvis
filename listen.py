def handle_command(command):
    ...
    if command == "wake up":
        # Launch Jarvis.py in a separate thread
        thread = Thread(target=lambda: run(["python", "Jarvis.py"]))
        thread.start()
...
def listen():
    ...
    try:
        ...
        try:
            driver.find_element(by=By.PATH, value="/html/body/div[1]/div").click()
        ...
        clear_button_xpath = (
            "/html/body/div[3]/section/div/div/div[2]/div/div[3]/div[2]/a[8]"
        )
        ...
        start_button_xpath = (
            "/html/body/div[3]/section/div/div/div[2]/div/div[3]/div[1]/a"
        )
    ...
    thread = Thread(target=lambda: run(["python", "Jarvis.py"]))
    thread.start()
    thread = Thread(target=lambda: run(["python", "Battery_Alert.py"]))
    thread.start()
    ...
    while True:
        ...
        if len(text) == 0:
            ...
            output_file_path = r"C:\Users\sachi\Jarvis\output_file.txt"
            with open(output_file_path, "w") as file_write:
                file_write.write(text)
            if "wake up" in text:
                handle_command("wake up")
